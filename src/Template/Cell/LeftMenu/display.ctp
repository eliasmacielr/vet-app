<aside class="main-sidebar">
    ...
    <section class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li <?= $controller === 'Customers' ? 'class="active"' : '' ?>><a href="<?= $this->Url->build(['controller' => 'Customers', 'action' => 'index']) ?>"><i class="fa fa-users"></i> <span>Clientes</span> <span class="label label-primary pull-right"><?= $customersCount ?></span></a></li>
            <li <?= $controller === 'Patients' ? 'class="active"' : '' ?>><a href="<?= $this->Url->build(['controller' => 'Patients', 'action' => 'index']) ?>"><i class="fa fa-stethoscope"></i> <span>Pacientes</span> <span class="label label-primary pull-right"><?= $patientsCount ?></span></a></li>
            <?php if (in_array($user['group_name'], ['super-admin', 'admin'])): ?>
                <li <?= $controller === 'Users' ? 'class="active"' : '' ?>><a href="<?= $this->Url->build(['controller' => 'Users', 'action' => 'index']) ?>"><i class="fa fa-users"></i> <span>Usuarios</span> <span class="label label-primary pull-right"><?= $usersCount ?></span></a></li>
            <?php endif; ?>
            ...
            <li <?= $controller === 'Vaccinations' && $action === 'showExpired' ? 'class="active"' : '' ?>><a href="<?= $this->Url->build(['controller' => 'Vaccinations', 'action' => 'showExpired']) ?>"><i class="fa fa-home"></i> <span>Vencidos</span> <span class="label label-primary pull-right"><?= $expiredCount >= 999 ? '999+' : $expiredCount ?></span></a></li>
